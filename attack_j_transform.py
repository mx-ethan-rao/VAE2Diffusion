...
def build_dataloaders(dataset: str, root: str, split_file: str, img_size: int, in_channels: int,
    ...
    mia_train_idxs = split["mia_train_idxs"].tolist()[:1000]
    mia_val_idxs = split["mia_eval_idxs"].tolist()[:1000]
...
def precompute_jacobians(decoder, Z):
    """
    Compute full Jacobians J(z) for every sample in Z.
    Z: (N, C, H, W) latent batch
    Returns list of Jacobians [J1, J2, ..., JN],
    each with shape (M, D) where:
      M = output dimension (flattened image)
      D = latent dimension (flattened z).
    """
    decoder.eval()
    J_list = []
    for i in tqdm(range(Z.size(0))):
        zi = Z[i:i+1].detach().requires_grad_(True)

        def f(z_in):
            return decoder.decode(z_in).reshape(-1)  # flatten image

        Ji = jacobian(f, zi, vectorize=True)   # (M, D)
        Ji = Ji.reshape(Ji.shape[0], -1).detach().cpu()
        J_list.append(Ji)

    return J_list

def apply_precomputed_J(J_list, V, out_shape):
    """
    Multiply stored Jacobians with corresponding vectors.
    J_list: list of (M, D)
    V: (N, D) flattened RHS (e.g. UNet outputs)
    out_shape: (C, H, W) of decoder output
    """
    outs = []
    for Ji, vi in zip(J_list, V):
        yi = Ji @ vi.view(-1, 1)     # (M,1)
        outs.append(yi.view(out_shape).to(vi.device))
...
def probe_unet(
    ...
    """
    Membership probe using J_z @ UNet(z, t) instead of raw UNet(z, t).
    If `decoder` is None, falls back to raw UNet output.
    """
    unet.eval()

    # Precompute latents to avoid re-encoding for each t
    Zm_list, Zn_list = [], []
    for (m, _), (n, _) in tqdm(zip(train_loader, val_loader)):
        m = m.to(device)
        n = n.to(device)
        with torch.no_grad():
    ...
    out_shape = decoder.decode(Zm[:1]).shape[1:]   # (C, H, W)
    Jm_list = precompute_jacobians(decoder, Zm)
    Jn_list = precompute_jacobians(decoder, Zn)
    print("Appened!")
    for tval in tqdm(probe_ts):
        ...
        # --- forward UNet on (clean) latents at probe time t
        # (this mirrors your original attack behavior)
        pred_m_lat = unet(Zm, t_m)
        pred_n_lat = unet(Zn, t_n)

        # --- apply decoder Jacobian if provided: pred = J_z @ pred_lat
        if decoder is not None:
            pred_m = apply_precomputed_J(Jm_list, pred_m_lat, out_shape)
            pred_n = apply_precomputed_J(Jn_list, pred_n_lat, out_shape)
        else:
            pred_m = pred_m_lat
            pred_n = pred_n_lat

        # --- scoring (same as before, but now in image space if decoder is used)
...
def main():
    ...
    if args.unet_vae_ckpt and args.vae_ckpt:
        ...
        probe_unet(
            ...
            decoder=vae_p,   # <-- J_z from the VAE decoder
        )
    ...
    if args.unet_vq_ckpt and args.vqvae_ckpt:
        ...
        probe_unet(
            ...
            train_loader, val_loader, device,
            probe_min_t=args.probe_min_t, probe_max_t=args.probe_max_t, probe_step=args.probe_step,
            decoder=vqvae_p,  # <-- J_z from the VQ-VAE decoder
        )

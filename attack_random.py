...
def build_dataloaders(dataset: str, root: str, split_file: str, img_size: int, in_channels: int,
    ...
    mia_train_idxs = split["mia_train_idxs"][random.sample(range(0, 30000), 2000)].tolist()
    mia_val_idxs = split["mia_eval_idxs"][random.sample(range(0, 30000), 2000)].tolist()
...
def random_partition_subset_loader(orig_loader, frac=0.5, seed=42):
    """
    Randomly split a Subset-backed DataLoader's dataset into two disjoint parts.
    Returns (loader_A, loader_B) with the SAME batch_size/num_workers as orig_loader.
    """
    assert isinstance(orig_loader.dataset, Subset), "random_partition_subset_loader expects a Subset-backed loader."
    subset: Subset = orig_loader.dataset
    base_indices = np.array(subset.indices)
    N = len(base_indices)

    rng = np.random.RandomState(seed)
    perm = rng.permutation(N)
    k = int(round(frac * N))
    idxA = base_indices[perm[:k]]
    idxB = base_indices[perm[k:]]

    dsA = Subset(subset.dataset, idxA.tolist())
    dsB = Subset(subset.dataset, idxB.tolist())

    loader_kwargs = dict(
        batch_size=orig_loader.batch_size,
        num_workers=orig_loader.num_workers,
        pin_memory=True,
        shuffle=False,  # keep deterministic order for probing
    )
    return DataLoader(dsA, **loader_kwargs), DataLoader(dsB, **loader_kwargs)
...
def main():
    ...
    train_r1_loader, train_r2_loader = random_partition_subset_loader(train_loader, frac=0.5, seed=args.seed + 101)
    val_r1_loader,   val_r2_loader   = random_partition_subset_loader(val_loader,   frac=0.5, seed=args.seed + 202)

    # ========== VAE path (if available) ==========
    if args.unet_vae_ckpt and args.vae_ckpt:
        print("\n[Random split probe] LDM-VAE path — Group 1")
        probe_unet(
            unet_vae, lambda x: get_latents_vae(vae_p, x),
            train_r1_loader, val_r1_loader, device,
            probe_min_t=args.probe_min_t, probe_max_t=args.probe_max_t, probe_step=args.probe_step,
        )

        print("\n[Random split probe] LDM-VAE path — Group 2")
        probe_unet(
            unet_vae, lambda x: get_latents_vae(vae_p, x),
            train_r2_loader, val_r2_loader, device,
            probe_min_t=args.probe_min_t, probe_max_t=args.probe_max_t, probe_step=args.probe_step,
        )
    else:

...
# =============================
# VQGAN-style GAN helpers (no perceptual)
# =============================
...
def g_nonsaturating_loss(logits_fake, use_hinge=True):
    if use_hinge:
        return -logits_fake.mean()  # hinge variant
    return F.softplus(-logits_fake).mean()  # logistic variant
...
def train_vae_loop(model, train_loader, epochs, lr, betas, wd, out_dir, device, kl_weight,
                   # --- GAN extras ---
                   gan_enable=False,
                   disc_start=10000,         # when GAN kicks in
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, _ in train_loader:
            ...
            # reconstruction + KL
            nll = recon_loss(x, recon) + kl_weight * kld_loss(mu, logvar_)

            adv_g = torch.tensor(0.0, device=device)
            d_weight = torch.tensor(0.0, device=device)

            disc_factor = torch.tensor(
                adopt_weight(1.0, global_step, threshold=disc_start, value=0.0),
                device=device
            )
...
def parse_args():
    ...
    # ====== GAN trick toggles for VAE path (no perceptual) ======
    p.add_argument("--gan_enable", action="store_true", help="Enable PatchGAN + adaptive d_weight on VAE training")
...
def main():
    ...
    if not args.skip_vae:
        ...
        train_vae_loop(
            ...
            disc_type=args.disc_type, d_ndf=args.d_ndf, d_layers=args.d_layers
        )

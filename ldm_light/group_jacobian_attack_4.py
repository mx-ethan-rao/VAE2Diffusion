...
def group_and_probe_by_logvol_quartiles(
    ...
    all_logv = torch.cat([logv_train, logv_val], dim=0).cpu().numpy()
    q25, q50, q75 = np.percentile(all_logv, [25, 50, 75])
    print(f"Quartile thresholds: 25%={q25:.6f}, 50%={q50:.6f}, 75%={q75:.6f}")
    ...
    t_q1, t_q2, t_q3, t_q4 = assign_bins(logv_train)
    v_q1, v_q2, v_q3, v_q4 = assign_bins(logv_val)

    counts = {
        "train_Q1": int(t_q1.sum()), "val_Q1": int(v_q1.sum()),
        "train_Q2": int(t_q2.sum()), "val_Q2": int(v_q2.sum()),
        "train_Q3": int(t_q3.sum()), "val_Q3": int(v_q3.sum()),
        "train_Q4": int(t_q4.sum()), "val_Q4": int(v_q4.sum()),
    }
    print("Group counts:", counts)

    # 4. build loaders
    def make_loader(loader, mask):
        return build_subset_loader_from_mask(loader, torch.tensor(mask), batch_size=loader.batch_size)

    loaders = [
        (make_loader(train_loader, t_q1), make_loader(val_loader, v_q1), "Q1 (lowest 25%)"),
        (make_loader(train_loader, t_q2), make_loader(val_loader, v_q2), "Q2 (25–50%)"),
        (make_loader(train_loader, t_q3), make_loader(val_loader, v_q3), "Q3 (50–75%)"),
        (make_loader(train_loader, t_q4), make_loader(val_loader, v_q4), "Q4 (highest 25%)"),
    ]

    # 5. run probing on each group
    for tr_loader, va_loader, name in loaders:
        print(f"\n--- Probing {name} ---")
        if len(tr_loader.dataset) == 0 or len(va_loader.dataset) == 0:
            print(f"[Skip] Empty dataset for {name}")
            continue
        probe_unet(unet, lat_fn, tr_loader, va_loader, device,
                   probe_min_t=probe_min_t, probe_max_t=probe_max_t, probe_step=probe_step)

    return counts, (q25, q50, q75), (logv_train.cpu().numpy(), logv_val.cpu().numpy())
...
def main():
    ...
    if args.unet_vae_ckpt and args.vae_ckpt:
        ...
        counts, quarts, logs = group_and_probe_by_logvol_quartiles(
        ...
        print("Quartile counts:", counts, "thresholds:", quarts)
    ...
    if args.unet_vq_ckpt and args.vqvae_ckpt:
        ...
        counts, quarts, logs = group_and_probe_by_logvol_quartiles(
            decoder=vqvae_p, unet=unet_vq, lat_fn=lambda x: get_latents_vae(vae_p, x),
            train_loader=train_loader, val_loader=val_loader, device=device,
            probe_min_t=args.probe_min_t, probe_max_t=args.probe_max_t, probe_step=args.probe_step,
            max_samples_per_split=None,
        )

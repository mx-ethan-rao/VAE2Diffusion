...
Additionally, this version reads per-dimension contribution NPZ and
drops a fraction (drop_percent) of the lowest-contribution latent coordinates
per sample when forming the attack statistic.
...
Usage example:
  python attack_grouped.py \
    ...
    --perdim_npz  runs/ldm_imnet256/imnetv1_3k_perdim.npz \
    --grouping quartiles \
    --drop_percent 10 \
    --batch_size 128 \
...
# original probe_unet (kept for reference)
@torch.no_grad()
def probe_unet(
...
# ---------------------------
# per-dim NPZ loading & alignment helper
# ---------------------------
def load_perdim_npz(npz_path: str) -> Dict[str, np.ndarray]:
    ...
    m_per = pick(['member_per_dim_logcontrib', 'member_per_dim', 'per_dim_logcontrib_train'])
    h_per = pick(['heldout_per_dim_logcontrib', 'heldout_per_dim', 'per_dim_logcontrib_val'])
    if m_idx is None or h_idx is None or m_per is None or h_per is None:
        raise KeyError(
            f"Per-dim NPZ missing required arrays. Found keys={list(data.keys())}. "
            "Expected indices (member/heldout) and per-dim arrays."
        )
    return {
        'member_indices': np.array(m_idx),
        'heldout_indices': np.array(h_idx),
        'member_perdim': np.array(m_per),
        'heldout_perdim': np.array(h_per),
    }

def build_group_perdim_arrays(
    group_indices: np.ndarray,
    all_indices: np.ndarray,
    all_perdim: np.ndarray,
) -> np.ndarray:
    """
    For a given group (list of dataset indices), return a per-dim array (N_group, D)
    aligned to the iteration order of the group's DataLoader (which is the same
    as the given group_indices with shuffle=False).
    """
    # map dataset index -> row position in per-dim array
    pos_map = {int(idx): i for i, idx in enumerate(all_indices.tolist())}
    rows = [pos_map[int(i)] for i in group_indices.tolist()]
    return all_perdim[rows, :]

# ---------------------------
# streaming probe with per-dim drop mask
# ---------------------------
@torch.no_grad()
def probe_unet_streaming_masked(
    unet: UNet2DConditionModel,
    class_embed: torch.nn.Embedding,
    vae: AutoencoderKL,
    member_loader: DataLoader,
    heldout_loader: DataLoader,
    contrib_member_group: np.ndarray,   # (Nm, D)
    contrib_held_group: np.ndarray,     # (Nn, D)
    drop_percent: float,
    device: torch.device,
    probe_min_t: int, probe_max_t: int, probe_step: int,
    SCALE: float = 0.18215,
    ...
    drop the lowest `drop_percent` contribution dimensions per sample.
    """
    ...
    probe_ts = list(range(probe_min_t, probe_max_t, probe_step))
    print(f"[Probe-stream(masked)] timesteps: {probe_ts} | drop_percent={drop_percent}")

    # containers: dict tval -> list of numpy floats
    sm_store = {t: [] for t in probe_ts}  # members
    sn_store = {t: [] for t in probe_ts}  # heldout

    # clamp drop_percent to [0, 100)
    dp = float(max(0.0, min(99.999, drop_percent)))

    def process_loader(loader, store_dict, contrib_group: np.ndarray, is_member: bool):
        cursor = 0  # tracks rows used from contrib_group to align with loader order
        for imgs, labs in tqdm(loader, desc=f"stream encode {'members' if is_member else 'heldout'}"):
            B = imgs.size(0)
            # per-batch per-dim contribs (B, D)
            contrib_batch = contrib_group[cursor:cursor + B, :]  # numpy (B, D)
            cursor += B
            ...
            for tval in probe_ts:
                ...
                #     eps_init = unet(z, torch.full((z.size(0),), 0, device=device, dtype=torch.long), encoder_hidden_states=ctx).sample
                    zt = a * z + b * eps_init
                    eps_hat = unet(zt, t_tensor, encoder_hidden_states=ctx).sample
                # pred = unet(z, t_tensor, encoder_hidden_states=ctx).sample
                # pred_f = (pred.abs() ** 4).flatten(1)  # (B, D)

                # apply mask (drop lowest dp%): elementwise multiply
                    eps_diff += (eps_hat - eps_init)
                eps_diff = eps_diff / n_mc
                s = ((eps_diff.flatten(1) ** 2)*mask).sum(-1)
                # s = (pred_f * mask).sum(dim=-1)

                s_np = s.detach().cpu().float().numpy()
                store_dict[tval].append(s_np)

    # process member and heldout with their aligned contrib arrays
    process_loader(member_loader, sm_store, contrib_member_group, is_member=True)
    process_loader(heldout_loader, sn_store, contrib_held_group, is_member=False)
...
def main():
    ...
    ap.add_argument("--perdim_npz", type=str, required=True, help="NPZ file with per-dimension log contributions")
    ...
    ap.add_argument("--probe_min_t", type=int, default=200)
    ap.add_argument("--probe_max_t", type=int, default=300)
    ap.add_argument("--probe_step", type=int, default=10)
    ap.add_argument("--grouping", type=str, choices=['random','median','quartiles'], default='random')
    ap.add_argument("--attack", type=str, choices=['loss','pia'], default='loss')
    ap.add_argument("--random_groups", type=int, default=1)
    ap.add_argument("--drop_percent", type=float, default=0.0, help="percent of lowest-contrib dims to drop per sample")
    ...
    # --- grouping based on pullback NPZ ---
    splits = build_groups_from_npz(args.pullback_npz, args.grouping, seed=args.seed, n_random_groups=args.random_groups)

    # --- load per-dimension contributions NPZ ---
    perd = load_perdim_npz(args.perdim_npz)
    m_idx_pd, h_idx_pd = perd['member_indices'], perd['heldout_indices']
    m_perdim_all, h_perdim_all = perd['member_perdim'], perd['heldout_perdim']
    D = m_perdim_all.shape[1]
    print(f"[Per-dim] loaded: members {m_perdim_all.shape}, heldout {h_perdim_all.shape}")

    # For each group: build loaders, align per-dim arrays to group order, run masked probe
    for name, mem_idx, held_idx in zip(splits['names'], splits['member_groups'], splits['held_groups']):
        ...
        print(f"\n=== Group {name} ===")
        # Per-group per-dim arrays aligned to Subset/DataLoader order
        contrib_mem_group = build_group_perdim_arrays(mem_idx, m_idx_pd, m_perdim_all)      # (Nm, D)
        contrib_held_group = build_group_perdim_arrays(held_idx, h_idx_pd, h_perdim_all)    # (Nn, D)
        ...
        # STREAMING masked probe
        betas = torch.linspace(1.5e-3, 1.95e-2, steps=1000, device=device)
        alpha_bar = torch.cumprod(1 - betas, dim=0).float()
        probe_unet_streaming_masked(
            unet=unet, class_embed=class_embed, vae=vae,
            member_loader=m_loader, heldout_loader=h_loader,
            contrib_member_group=contrib_mem_group,
            contrib_held_group=contrib_held_group,
